...
const useProduct = defineStore('product', () => {
  ...
  const getCartProducts = async () => {
    try {
      ...
      const getIdProducts = cartProducts.value.map((product) => `id-${product.id}`);
      ...
      const transformCarts = quantityProducts.value
        .map((item) => {
          ...
          const { id, title, image, price } = products.find((product) => product.id === item.productId);
      ...
      cartProducts.value.splice(0, cartProducts.value.length, ...transformCarts);
    } catch (error) {
      console.error('Error get product in cart:', error);
      alert('There was an error get the product to the cart. Please try again later (;');
    }
  };
  ...
  const addToCart = async (productId, quantity) => {
    try {
      ...
      const getIdProducts = response.products.map((product) => product.productId);
  ...
  const deleteProductCart = async (productId) => {
    try {
      ...
      const deleteQuantityProduct = quantityProducts.value.filter((item) => item.productId !== id);

      quantityProducts.value.splice(0, quantityProducts.value.length, ...deleteQuantityProduct);
